...
try {
  ...
  describe('Test Schedules', () => {
    it('should create a Schedule object successfully', () => {
      ...
      assert(/[0-9]{4}-[0-9]{2}-[0-9]{2}/.test(date.date), 'returned date from schedule should be a YYYY-MM-DD date');
      assert(Moment.isMoment(Moment(date.openingTime)), 'returned openingTime from schedule should be a Moment parsable date/time');
      assert(Moment.isMoment(Moment(date.closingTime)), 'returned closingTime from schedule should be a Moment parsable date/time');
    ...
    it('should cache Schedule object data correctly', (done) => {
      ...
      sched.WriteToCache().then(() => {
        ...
        sched2.ReadFromCache().then(() => {
          ...
          assert(/[0-9]{4}-[0-9]{2}-[0-9]{2}/.test(cachedDate.date), 'returned date from cached schedule should be a YYYY-MM-DD String');
          assert(Moment.isMoment(Moment(cachedDate.openingTime)), 'returned openingTime from cached schedule should be a Moment parsable date/time');
          assert(Moment.isMoment(Moment(cachedDate.closingTime)), 'returned closingTime from cached schedule should be a Moment parsable date/time');
  ...
  // Test Ride object
  describe('Test Ride Obejct', () => {
    it('should update lastUpdate value when waitTime changes', (done) => {
      ...
      // set ride to 5 minutes
      TestRide.fromJSON({
        waitTime: 5,
      });

      const initialUpdateTime = TestRide.toJSON().lastUpdate;

      // set ride to 5 minutes again
      TestRide.fromJSON({
        waitTime: 5,
      });
      assert(TestRide.toJSON().lastUpdate === initialUpdateTime, 'Ride lastUpdate time should not change when the waitTime has not changed between updates');

      setTimeout(() => {
        // bump to 10 minutes and check
        TestRide.fromJSON({
          waitTime: 10,
        });
        assert(TestRide.toJSON().lastUpdate > initialUpdateTime, `Ride lastUpdate should update when waitTime changes. ${TestRide.toJSON().lastUpdate} !> ${initialUpdateTime}`);

        done();
      }, 1500); // wait briefly to ensure the time changes
    });

    it('should update lastUpdate when meta data is changed', (done) => {
      const TestRide = new Ride({
        rideId: 'testrideWithMetaData',
        timezone: 'Europe/London',
        forceCreate: true,
      });

      // set some initial meta data
      TestRide.fromJSON({
        waitTime: 5,
        meta: {
          returnTime: '09:00',
      ...
      setTimeout(() => {
        // set identical meta data to test
        TestRide.fromJSON({
          waitTime: 5,
          meta: {
            returnTime: '09:00',
        ...
        assert(TestRide.toJSON().lastUpdate === initialUpdateTime, 'Setting same meta data should not change the lastUpdate time');
        ...
        setTimeout(() => {
          ...
          assert(TestRide.toJSON().lastUpdate > initialUpdateTime, `Setting changed meta data should update the lastUpdate time. ${TestRide.toJSON().lastUpdate} > ${initialUpdateTime}`);
