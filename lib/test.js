...
try {
  ...
  describe("Fetch an access token from the WDW API", function() {
    // Test WDW API network requests
    var WaltDisneyWorldPark = require("./disneyworld/waltdisneyworld.js");
    var park = new WaltDisneyWorldPark();
    // configure HTTP to use offline tests
    Debug.OfflineTests = park.SetupOfflineTests;

    it("should return an access token", function(done) {
      park.GetAccessToken().then(function(token) {
        assert(token, "Should return a valid access token");
        done();
      }, function(err) {
        assert(false, "Error fetching access token: " + err);
        done();
      });
    });
  });
