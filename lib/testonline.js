...
const PRINTDATA = process.env.PRINTDATA ? true : false;
...
function TestPark(park) {
    ...
    describe("Get Park Wait Times", function() {
        ...
        it("should have an ID for every ride", function() {
            for (let i = 0, ride; ride = times[i++];) ValidateType(ride, "id", ["string", "number"]);
        });

        it("should have a wait time for every ride", function() {
            for (let i = 0, ride; ride = times[i++];) ValidateType(ride, "waitTime", "number");
        });

        it("should have a name for every ride", function() {
            for (let i = 0, ride; ride = times[i++];) ValidateType(ride, "name", "string");
        });

        it("should have an active state for every ride", function() {
            for (let i = 0, ride; ride = times[i++];) ValidateType(ride, "active", "boolean");
        });

        it("should have a fastpass field for every ride", function() {
            for (let i = 0, ride; ride = times[i++];) {
