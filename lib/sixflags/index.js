    ...
    FetchOpeningTimes() {
        return new Promise(function(resolve, reject) {
            this.GetAPIUrl({
                ...
                // fill in missing dates as closed
                var endFillDate = Moment().add(60, "days");
                for (var m = Moment(); m.isBefore(endFillDate); m.add(1, "days")) {
                    if (!this.Schedule.GetDate(m)) {
                        this.Schedule.SetDate({
                            date: m,
                            openingTime: m.startOf("day"),
                            closingTime: m.endOf("day"),
                            type: "Closed"
                        });
                    }
                }
