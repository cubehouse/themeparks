    ...
    SetGlobal(key, data, ttl, stringify = true) {
        return GetDB().then(db => {
            return new Promise((resolve, reject) => {
                ...
                const ttlResolver = typeof ttl == "function" ? ttl : () => Promise.resolve(ttl);
            ...
            return new Promise((resolve, reject) => {
                ...
                Promise.all([ttl]).then((ttl) => {
                    ...
                    db.serialize(() => {
                        ...
                        for (let key in data) {
                            cacheInsertStatement.run(ignoreCacheKey ? key : this.Prefix + key, stringify ? JSON.stringify(data[key]) : data[key], ttl[0] + (Math.floor(new Date().getTime() / 1000)));
                        }
                        // commit the inserts
                        db.run("commit", () => {
