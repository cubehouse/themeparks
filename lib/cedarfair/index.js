    ...
    FetchOpeningTimes() {
        return new Promise(function(resolve, reject) {
            ...
            this.FetchStaticScheduleData().then(function(scheduleData) {
                ...
                for (var monthIdx = 0, month; month = scheduleData.main.months[monthIdx++];) {
                    for (var hoursIdx = 0, dayRow; dayRow = month.hours[hoursIdx++];) {
                        for (var dayIdx = 0, day; day = dayRow[dayIdx++];) {
                ...
                for (var specialHourIdx = 0, specialHours; specialHours = this[s_specialHours][specialHourIdx++];) {
                    ...
                    for (var specialMonthIdx = 0, month; month = scheduleData[specialHours].months[specialMonthIdx++];) {
                        for (var specialHoursIdx = 0, dayRow; dayRow = month.hours[specialHoursIdx++];) {
                            for (var specialDayIdx = 0, day; day = dayRow[specialDayIdx++];) {
                                // skip this entry if there is no day set
                                if (!day.day) continue;
                                // skip this entry if the class doesn't appear in the legend
