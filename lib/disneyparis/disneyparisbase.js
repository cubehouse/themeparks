...
class DisneyParisPark extends Park {
      ...
      data.data.activitySchedules.forEach((activity) => {
        ...
        if (activity.schedules && activity.schedules[0]) {
          ...
          if (openingData.status === 'OPERATING') {
            Attraction.openingTime = Moment.tz(activity.schedules[0].startTime, 'HH:mm:ss', this.Timezone).format();
            Attraction.closingTime = Moment.tz(activity.schedules[0].endTime, 'HH:mm:ss', this.Timezone).format();
          }
        }
  ...
  GetParkData() {
    ...
    return Cache.WrapGlobal('dlp_ridedata_20191013', this.FetchParkData.bind(this), 60 * 60 * 4); // fetch once every 4 hours
  }
