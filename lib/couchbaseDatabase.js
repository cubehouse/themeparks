            ...
            if (boundaryMatch && boundaryMatch[1]) {
                ...
                while ((JSONMatch = regexSplit.exec(respBody)) !== null) {
                    ...
                    try {
                        ...
                        this.emit("error", `Error parsing couchbase JSON response: ${JSONMatch[1]}: ${e}`);
                    }
                }

                // emit new documents
                for (let i = 0, doc; doc = JSONResponses[i++];) {
...
const test = new CouchbaseLiteDatabase("https://realtime-sync-gw.wdprapps.disney.com/park-platform-pub", "wdw.facilitystatus.1_0");
test.Auth = {
