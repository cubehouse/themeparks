...
const sSix = Symbol('Use 6.x Preview Servers');
...
class DisneyPark extends Park {
  constructor(options = {}) {
    ...
    this[sSix] = options.six || false;
    if (this[sSix]) return; // skip setting up channels if we're in "6" mode
  ...
  FetchWaitTimes() {
    ...
    if (this[sSix]) {
      return this.HTTP({
        url: `https://api.themeparks.wiki/preview/parks/${this.constructor.name}/waittime`,
      }).then((data) => {
        data.forEach((ride) => {
          this.UpdateRide(ride.id, ride);
        });
      });
    }
  ...
  FetchOpeningTimes() {
    ...
    if (this[sSix]) {
      return this.HTTP({
        url: `https://api.themeparks.wiki/preview/parks/${this.constructor.name}/calendar`,
      }).then((data) => {
        data.forEach((date) => {
