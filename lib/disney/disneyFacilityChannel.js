...
const sTimezone = Symbol('Timezone for this channel');

function CalendarDocDate(doc, timezone) {
...
class FacilityChannel {
  constructor(options = {}) {
    ...
    this[sTimezone] = options.timezone || 'Etc/UTC';
  }
    ...
    if (facilityDoc) {
      ...
      if (!this.Synced) {
        this[sCalendarDBWrite].push({
          date: CalendarDocDate(doc, this[sTimezone]),
          /* eslint-disable no-underscore-dangle */
          docKey: doc._id,
          /* eslint-enable no-underscore-dangle */
        });
  ...
  StoreCalendarDocSingle(doc) {
    Cache.DB().then((db) => {
      ...
      const docDate = CalendarDocDate(doc, this[sTimezone]);
      ...
      db.all(SQL, SQLValues, (err, rows) => {
        ...
        rows.forEach((row) => {
          ...
          try {
            ...
            JSONData.parsedDate = CalendarDocDate(JSONData, this[sTimezone]);
