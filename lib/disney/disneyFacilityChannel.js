...
const sStartCallbacks = Symbol('Initial Callbacks');
...
class FacilityChannel {
  constructor(options = {}) {
    ...
    this[sUseOffline] = options.use_offline || false;
  ...
  Start() {
    ...
    if (this[sStarted]) {
      return new Promise((resolve) => {
        ...
        return this[sStartCallbacks].push(resolve);
      });
    }
    this[sStarted] = true;

    // start channel, then bulk store new docs before marking as synced
    return this[sChannel].Start().then(this.BulkUpdateFacilityData.bind(this)).then(this.BulkUpdateCalendarData.bind(this)).then(() => {
      // mark ourselves as synced and return
      this[sSynced] = true;

      // call any callbacks from further requests for Start
      this[sStartCallbacks].forEach((cb) => {
        cb();
      });
      this[sStartCallbacks] = [];
