        ...
        return this.Initialise().then(() => {
            ...
            return Cache.DB().then((db) => {
                return new Promise((resolve, reject) => {
                    db.get("SELECT C2.key, C2.value FROM cache AS C1 INNER JOIN cache AS C2 ON C2.key = trim(C1.value,'\"') WHERE C1.key = ?;", [this.Cache.Prefix + TidyDisneyID(facilityID)], (err, row) => {
                        if (err) {
...
if (!module.parent) {
    ...
    Fac.GetFacilityData(80007944).then((doc) => {
