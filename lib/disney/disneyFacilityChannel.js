...
class FacilityChannel {
    constructor(options = {}) {
        ...
        this[s_Channel] = new CouchbaseChannelDisney({
            dbName: `facilities_${options.park_id}`,
            channel: `${options.park_id}.facilities.${options.version === undefined ? "1_0" : options.version}.${options.version === undefined ? "en_us" : options.version}`
        });
        ...
        this[s_Cache] = new Cache({
            prefix: `facilitychannel_${options.park_id}`
        });
        ...
        return this.Initialise().then(() => {
            ...
            return Cache.DB().then((db) => {
                return new Promise((resolve, reject) => {
                    ...
                    db.get("SELECT C.body FROM disneyFacilities AS F INNER JOIN couchbasesync AS C ON C.id = F.docKey WHERE F.id = ? AND C.dbName = ?;", [tidyID, "facilities_" + this[s_ParkID]], (err, row) => {
                        if (err) {
                        ...
                        try {
                            ...
                            return reject(`Unable to parse JSON object for facility ${facilityID} :: ${row.body}`);
                        }
