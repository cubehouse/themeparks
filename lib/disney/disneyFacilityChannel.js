            ...
            return new Promise((resolve) => {
                ...
                db.serialize(() => {
                    ...
                    for (let i = 0, q; q = this[s_FacilityCacheDBWrite][i++];) {
                        cacheInsertStatement.run(q.id, q.name, q.entityType, q.docKey, this[s_ResortID]);
                    }
        ...
        return this.Initialise().then(() => {
            ...
            return Cache.DB().then((db) => {
                return new Promise((resolve, reject) => {
                    ...
                    db.get("SELECT C.body FROM disneyFacilities AS F INNER JOIN couchbasesync AS C ON C.id = F.docKey WHERE F.id = ? AND C.dbName = ?;", [tidyID, "facilities_" + this[s_ResortID]], (err, row) => {
                        if (err) {
