                ...
                for (var i = 0, schedule; schedule = scheduleData.activities[i++];) {
                    ...
                    for (var j = 0, scheduleTime; scheduleTime = schedule.schedule.schedules[j++];) {
                        ...
                        if (scheduleID == this.WDWParkID) {
                            ...
                            // find the ride object (this will return null if it doesn't exist)
                            var rideObject = this.FindRideObject({
                                id: scheduleID
                            });

                            // if we found this ride in our list, update it's schedule data
                            if (rideObject) {
                                rideObject.Schedule.SetDate(newScheduleData);
                            }
                        }
                    }
                }
