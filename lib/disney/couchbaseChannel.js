...
const sHasInitialStateCallbacks = Symbol('Initial State Callbacks Present?');
...
class CouchbaseChannel extends EventEmitter {
  constructor(options = {}) {
    ...
    this[sHasInitialStateCallbacks] = [];
  ...
  Start() {
    ...
    if (this[sLongPollActive]) {
      return new Promise((resolve) => {
        // if we've become synced before this Promise resolved, resolve now!
        if (this[sSynced]) {
          return resolve();
        }
        return this[sHasInitialStateCallbacks].push(resolve);
      });
    }
    ...
    return this.GetInitialState().then(() => {
      ...
      this[sHasInitialStateCallbacks].forEach((cb) => {
        process.nextTick(cb);
      });
      this[sHasInitialStateCallbacks] = [];
