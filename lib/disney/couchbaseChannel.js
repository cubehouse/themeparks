  ...
  ProcessDocChanges(documents) {
    ...
    documents.forEach((doc) => {
      if (doc.deleted) {
        ...
        const docDeleteID = RevisionToInt(doc.changes[0].rev);

        // delete from pending fetches if we're about to fetch this doc revision (but this delete request overrides it)
        docRequest.forEach((docRequested, idx) => {
          if (docDeleteID >= RevisionToInt(docRequested.rev)) {
            docRequest.splice(idx, 1);
          }
        });
        ...
        const newDocRevID = RevisionToInt(doc.changes[0].rev);

        // remove from delete list if our new revision is higher than any deletion requests
        docDeletes.forEach((deleteDoc, idx) => {
          if (newDocRevID > RevisionToInt(deleteDoc.rev)) {
            docDeletes.splice(idx, 1);
          }
        });
        ...
        docRequest.forEach((docRequested) => {
          if (docRequested.id === doc.id) {
            ...
            if (newDocRevID > RevisionToInt(docRequested.rev)) {
      ...
      docDeletes.forEach((x) => {
        console.log(`Delete Doc ID: ${x.id} revision ${x.rev}`);
