...
class CouchbaseChannel extends EventEmitter {
    ...
    /**
     * Get the initial channel state, return Promise, then start live updates
     * You'll probably want to call this, so you know you have latest data
     */
    GetInitialStateThenLongPoll() {
        if (this[s_longPollActive]) return Promise.resolve();

        // TODO - if multiple things access this before we're ready, keep track of them to call their resolves
    ...
    ProcessDocChanges(documents) {
        ...
        while (documents.length > 0) {
            batches.push(documents.splice(0, Math.min(docFetchBatchSize, documents.length)));
        }
    ...
    _INT_ProcessDocChangeBatch(documents) {
...
const test = new CouchbaseChannel("https://realtime-sync-gw.wdprapps.disney.com/park-platform-pub", "ridestatus", "wdw.facilitystatus.1_0");
test.Auth = {
...
//test.Start();
/*test.GetInitialStateThenLongPoll().then(() => {
