    ...
    GetInitialState() {
        ...
        return this.GetLastSequence().then((lastSequenceID) => {
            ...
            return needle("POST", `${this.BaseURL}/_changes?feed=normal&heartbeat=30000&style=all_docs&filter=sync_gateway%2Fbychannel`, {
                ...
                this.Log(`Initial state returned ${resp.body.results.length} updated documents`);
    ...
    FetchDocuments(docRequest) {
        ...
        return needle("POST", `${this.BaseURL}/_bulk_get?revs=true&attachments=true`, {
            ...
            if (boundaryMatch && boundaryMatch[1]) {
                const regexString = `${boundaryMatch[1]}\\s*Content-Type:\\s+application\\/json[^\\n]*\\n+(.*)`;
                    ...
                    try {
                        ...
                        if (JSONData.error !== undefined) {
                            if (JSONData.reason !== undefined && JSONData.reason == "missing") {
                                ...
                                this.emit("error", `Requested missing document revision: ${JSONData.id}`);
                            } else {
                                this.emit("error", `Error fetching couchbase document: ${JSONData.error} / ${JSONData.id}`);
                            }
                        } else {
