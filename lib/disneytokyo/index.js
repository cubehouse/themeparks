            ...
            function() {
                return new Promise(
                    function(resolve, reject) {
                        ...
                        var randomGeoLocation = GeoLocation.RandomBetween(
                            this[s_parkLocationMin],
                            this[s_parkLocationMax]
                        );
                        ...
                        this.HTTP({
                            ...
                            url: `https://www.tokyodisneyresort.jp/${
                                this[s_parkID]
                            }/realtime.html?nextUrl=${this[s_parkID]}attraction`,
                            headers: {
                                connection: "keep-alive"
                            },
                            retryDelay: 1000 * 10
                            ...
                            if (!match)
                                return reject(
                                    "Unable to extract blockId and pageBlockId from Tokyo Disneyland page"
                                );

                            // request cookie for accessing wait times using a random location in the park
                            this.HTTP({
                                method: "POST",
                                url: `https://www.tokyodisneyresort.jp/view_interface.php?nextUrl=${
                                    this[s_parkID]
                                }attraction&blockId=${match[1]}&pageBlockId=${match[2]}`,
                                data: {
                                ...
                                function(resp) {
                                    if (resp.body && resp.body.result === false) {
                                        return reject(
                                            "Tokyo Disney Resort failed our location test"
                                        );
                                    }
                                    ...
                                    if (
                                        ...
                                        var GPSCookie,
                                            GPSExpiresIn = 60 * 30;
                                        for (
                                            var i = 0, cookie_string;
                                            (cookie_string = resp.headers["set-cookie"][i++]);
                                        ...
                                        if (GPSCookie) {
                                            ...
                                            return reject(
                                                "Failed to find GPS Cookie from TDR website"
                                            );
                                        }
                                    } else {
